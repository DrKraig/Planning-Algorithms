...
class Graph:
    ...
    def performDijkstra(self, start, end):
        ...
        self.priorityQueue = [start]
        while len(self.priorityQueue):
            currentNode = self.priorityQueue.pop(0)
            ...
            for neighbourNode, newDistance in neighbours.items():
                ...
                self.insertMy(neighbourNode)
        print("Cannot find a path :(")
        return False


    def insertMy(self, node):
        if self.size() == 0:
        ...
        # traverse the queue to find the right place for new node
            for x in range(0, self.size()):
                ...
                if node.distanceToReach >= self.priorityQueue[x].distanceToReach:
                # if we have traversed the complete queue
                    if x == (self.size()-1):
                    # add new node at the end
                        self.priorityQueue.insert(x+1, node)
                    else:
                        continue
                else:
                    self.priorityQueue.insert(x, node)
                    return True
    ...
    def visualizeDijkstra(self, start, end):
        ...
        self.priorityQueue = [start]
        while len(self.priorityQueue):
            currentNode = self.popFromPriorityQueue()
            ...
            for neighbourNode, newDistance in currentNode.neighbours.items():
                ...
                self.priorityQueue.append(neighbourNode)
        return 
...
if robot.performDijkstra(start, end):
    ...
    # pygame.display.set_caption("Dijkstra's Algorithm - Project3")
    # exiting = False
